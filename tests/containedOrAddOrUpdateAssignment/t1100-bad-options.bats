...
@test "error when no lhs and rhs passed" {
    run containedOrAddOrUpdateAssignment "$FILE"
    [ $status -eq 2 ]
...
@test "error when no rhs passed" {
    run containedOrAddOrUpdateAssignment --lhs foo "$FILE"
    [ $status -eq 2 ]
    [ "${lines[0]}" = "ERROR: Both -l|--lhs ASSIGNEE and -r|--rhs VALUE must be passed." ]
    [ "${lines[1]%% *}" = "Usage:" ]
}
...
@test "error when no lhs passed" {
    run containedOrAddOrUpdateAssignment --rhs bar "$FILE"
    [ $status -eq 2 ]
    [ "${lines[0]}" = "ERROR: Both -l|--lhs ASSIGNEE and -r|--rhs VALUE must be passed." ]
    [ "${lines[1]%% *}" = "Usage:" ]
}
...
@test "error when combining --ignore-nonexisting and --create-nonexisting" {
    run containedOrAddOrUpdateAssignment --ignore-nonexisting --create-nonexisting --lhs foo --rhs new "$FILE"
    [ $status -eq 2 ]
